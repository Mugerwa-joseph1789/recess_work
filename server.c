...
void search_details(){
   ...
   // printf("Record returned", search_name);
    ...
    puts("\n****************************************");
    puts("\n");
    puts("Patient_Name\tGender\tDate\t\tStatus\tOfficer_Name");
    puts("\n");
       ...
       // get_command(display_commands());
        ...
        if (sick_file == NULL)
        {
            ...
            for (i = 0; i < number_of_patients; i++) {
                ...
                fprintf(sick_file, "%s\t %s\t %s\t %s\t %s\t %s\n", patients[i].patient_fname,patients[i].patient_sname,patients[i].patient_gender,patients[i].DOI, patients[i].status,User_name);
            }
            printf("\n------ All Patients Added ------\n");
 ...
 void add_patient(void){
    ...
    for (loop = number_of_patients; loop <= MAX_PATIENTS; loop++)
    {
        while(1){
         ...
         scanf("%s %s",a3,a4);
         if(strcmp(a1,a3)!=0 && strcmp(a2,a4)!=0 || strcmp(a1,a3)==0 && strcmp(a2,a4)!=0 || strcmp(a1,a3)!=0 && strcmp(a2,a4)==0){
